...
def initialize(directory, game_root, executable, *, character=None, count=1, excluded_seeds=()):
    if character is not None and character not in CHARACTERS:
        raise ValueError('Unknown exam character')
    if type(count) is not int or not 1 <= count <= 100 or (character is None and count != 1):
        raise ValueError('Use a single character and a seed count between 1 and 100')
    ...
    def seed():
        while True:
            ...
            if value not in used:
                used.add(value)
                return value
    cases = ([{'id': f'{character.lower()}-{i+1:02d}', 'character': character, 'seed': seed()} for i in range(count)]
             if character else [{'id': c.lower(), 'character': c, 'seed': seed()} for c in CHARACTERS])
    exam = {'schema': 1, 'id': safe_id(directory.name), 'created_at': utc(), 'ascension': 10,
            ...
            'rules': {'floor': 'native cumulative TotalFloor, including the entered fatal floor',
                ...
                'eligibility': f'Only completed official games; overall requires all {len(cases)} assigned cases. Unavailable enemy HP leaves the HP tie unresolved.',
                'one_attempt': True}}
...
def report(manifest):
    ...
    lines = ['# Slay the Spire 2 · A10 模型考场', '',
             f"同一组 {len(exam['cases'])} 道正式题目；所有参赛模型均在 Claude Code 内通过专用 MCP 操作。", '',
             '| 题号 | 角色 | 固定种子 |', '|---|---|---|']
    lines.extend(f"| {c['id']} | {c['character']} | `{c['seed']}` |" for c in exam['cases'])
    ...
    lines.extend(['', f"连通性试跑：每模型 {policy['pilot_seconds']} 秒，共用独立 Ironclad 练习种子。",
                  ...
                  '同题先比累计楼层，同层先分胜败；胜者比自身剩余生命比例（高优），败者比敌方剩余生命比例（低优）。',
                  '敌方百分比包含该战斗内已击杀与召唤的敌人。非战斗死亡无敌人血量时保留并列。',
                  f"总榜需要全部 {len(exam['cases'])} 局均自然结束，依次比较累计楼层之和、通关数、血量得分之和（胜局自身比例，败局 1−敌方比例）。",
...
def main():
    ...
    init = sub.add_parser('init', help='Roll/freeze official cases and one separate practice seed exactly once')
    init.add_argument('--character', choices=CHARACTERS, help='Optional single-character exam')
    init.add_argument('--count', type=int, default=1, help='Number of seeds for the selected character')
    init.add_argument('--exclude-exam', type=Path, action='append', default=[], help='Do not reuse seeds from these historical manifests')
    ...
    for name in ('pilot', 'run'):
        ...
        if name == 'run':
            p.add_argument('--case', default='all', help='Case id from the manifest, or all')
    ...
    try:
        if args.command == 'register':
            ...
            for path in args.exclude_exam:
                ...
                excluded.update(c['seed'] for c in prior['cases'])
                if prior.get('pilot'):
                    excluded.add(prior['pilot']['seed'])
            result = initialize(args.exam.parent, args.game_root, args.claude,
                                character=args.character, count=args.count, excluded_seeds=excluded)
            report(args.exam)
            ...
            assigned = [c['id'] for c in load_exam(args.exam)['cases']]
            cases = ['practice'] if args.command == 'pilot' else (assigned if args.case == 'all' else [args.case])
            if args.command == 'run' and any(c not in assigned for c in cases):
